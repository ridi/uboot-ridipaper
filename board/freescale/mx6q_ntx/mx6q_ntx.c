...
static void setup_uart(void)
{
	...
	/* UART2 TXD */
	mxc_iomux_v3_setup_pad(MX6Q_PAD_SD4_DAT7__UART2_TXD);

	/* UART2 RXD */
	mxc_iomux_v3_setup_pad(MX6Q_PAD_SD4_DAT4__UART2_RXD);
...
struct fsl_esdhc_cfg usdhc_emmc_cfg[2] = {
	{USDHC3_BASE_ADDR, 1, 1, 1, 0},
	{USDHC1_BASE_ADDR, 1, 1, 1, 0},
};
...
int usdhc_gpio_init(bd_t *bis)
{
	s32 status = 0;
	u32 index = 0;
	uint soc_sbmr = get_boot_device();
	uint soc_bt_slot = get_boot_slot();

	if (MMC_BOOT == soc_sbmr)
		printf ("[[[[[  Boot from eMMC from slot %d ]]]]]\n", soc_bt_slot);
	else
		printf ("[[[[[  Boot from SD from slot %d ]]]]]\n", soc_bt_slot);
	...
	for (index = 0; index < CONFIG_SYS_FSL_USDHC_NUM;
		...
		if(2 == soc_bt_slot) {
			...
			status |= fsl_esdhc_initialize(bis, &usdhc_emmc_cfg[index]);
		}
		else {
			switch (index) {
			case 0:
...
*could not be power off anymore*/
	...
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_MX6Q_NTX;
...
int board_late_init(void)
{
	...
	//printf("UISytle=%d\n",(int)gptNtxHwCfg->m_val.bUIStyle);
	if (!gptNtxHwCfg) {
		...
		if(2==gptNtxHwCfg->m_val.bUIStyle) {
			...
			case SD_BOOT:
				printf("boot from SD\n");
				setenv("bootcmd", "run bootcmd_droid");
				break;
			case MMC_BOOT:
				s = getenv("run_fastboot");
				if (NULL != s) {
					printf("enter fastboot\n");
					setenv("bootcmd", "destroyenv; fastboot q");
				}
				else {
...
{
	unsigned int reg;
	iomux_v3_cfg_t enet_reset = MX6Q_PAD_EIM_A19__GPIO_2_19;
...
int checkboard(void)
{
	printf("Board: %s-NTX: %s Board: 0x%x [",
	mx6_chip_name(),
	mx6_board_rev_name(),
	fsl_system_rev);
