...
u32 get_board_rev(void)
{
	...
	system_rev = 0x60000 | BOARD_REV_3; /* means revB: NTX */
...
iomux_v3_cfg_t usdhc2_8bits_pads[] = {
	/* boot SD */
	MX6SL_PAD_SD2_CLK__USDHC2_CLK,
	MX6SL_PAD_SD2_CMD__USDHC2_CMD,
	MX6SL_PAD_SD2_DAT0__USDHC2_DAT0,
...
iomux_v3_cfg_t usdhc4_pads[] = {
	...
	MX6SL_PAD_FEC_REF_CLK__USDHC4_RST,
};
...
int usdhc_gpio_init(bd_t *bis)
{
	...
	if(!gptNtxHwCfg) {
		...
		case 0:
			...
			break;
		case 1:
			mxc_iomux_v3_setup_multiple_pads(usdhc2_8bits_pads,
						ARRAY_SIZE(usdhc2_8bits_pads));
			break;
		case 2:
	...
	case 33: 
		...
		if(NTXHWCFG_TST_FLAG(gptNtxHwCfg->m_val.bPCB_Flags2,2)&&
				gptNtxHwCfg->m_val.bLAN) 
		{
			// eMMC@SD2 && LAN port enabled.
			if (1==iISD_IDX) {
				...
				// boot from ESD ...
				printf("HW switch boot from ESD port 1 .\n");
				mxc_iomux_v3_setup_multiple_pads(usdhc2_8bits_pads,
					ARRAY_SIZE(usdhc2_8bits_pads));
...
#define I2C3_SCL_GPIO3_21_BIT_MASK  (1 << 21)
#define I2C3_SDA_GPIO3_22_BIT_MASK  (1 << 22)
...
static void mx6sl_i2c_gpio_scl_direction(int bus, int output)
{
	...
	case 3:
		...
		reg = readl(GPIO3_BASE_ADDR + GPIO_GDIR);
		if (output)
			reg |= I2C3_SCL_GPIO3_21_BIT_MASK;
		else
			reg &= ~I2C3_SCL_GPIO3_21_BIT_MASK;
		writel(reg, GPIO3_BASE_ADDR + GPIO_GDIR);
		break;
...
static void mx6sl_i2c_gpio_sda_direction(int bus, int output)
{
	...
	case 3:
		...
		reg = readl(GPIO3_BASE_ADDR + GPIO_GDIR);
		if (output)
			reg |= I2C3_SDA_GPIO3_22_BIT_MASK;
		else
			reg &= ~I2C3_SDA_GPIO3_22_BIT_MASK;
		writel(reg, GPIO3_BASE_ADDR + GPIO_GDIR);
		break;
...
static int mx6sl_i2c_gpio_check_sda(int bus)
{
	...
	case 3:
		reg = readl(GPIO3_BASE_ADDR + GPIO_PSR);
		result = !!(reg & I2C3_SDA_GPIO3_22_BIT_MASK);
		break;
	}
...
int check_powerkey_pressed(void)
{
	...
	if(NTXHWCFG_TST_FLAG(gptNtxHwCfg->m_val.bPCB_Flags2,2)) {
		// eMMC@SD2 , 
		mxc_iomux_v3_setup_pad(MX6SL_PAD_ECSPI1_SCLK__GPIO_4_8);
		GPIO_POWER_KEY = IMX_GPIO_NR(4, 8);
	}
	else
	if(31==gptNtxHwCfg->m_val.bPCB||32==gptNtxHwCfg->m_val.bPCB||NTXHWCFG_TST_FLAG(gptNtxHwCfg->m_val.bPCB_Flags2,0)) 
	{
		...
		GPIO_POWER_KEY = IMX_GPIO_NR(5, 8);
	}
...
static const NTX_GPIO gt_ntx_gpio_KB_COL0= {
	MX6SL_PAD_KEY_COL0__GPIO_3_24,  // pin name .
	3, // gpio group .
	24, // gpio number .
...
//tmp add
	if(13 == gptNtxHwCfg->m_val.bBattery) {
		...
		ntx_gpio_init(&gt_ntx_gpio_ACTION_LED_gp5_10);
	}
//end of add
...
//#if defined( CONFIG_MX6SL_NTX ) //[
#if defined( _MX6SL_ ) //[
 	if(NTXHWCFG_TST_FLAG(gptNtxHwCfg->m_val.bPCB_Flags2,2)) {
		// eMMC@SD2 , IOs@SPI .
		gptNtxGpioWifiPwr = &gt_ntx_gpio_WIFI_3V3_ON_1_1;
		gptNtxGpioESDIN = &gt_ntx_gpio_esdin_5_9;
	}
	else 
#endif //]
...
int board_init(void)
{
	...
	iChk=ntx_gpio_init(&gt_ntx_gpio_KB_COL0);
	if(iChk!=0) {
		printf("%s():KB_COL0 gpio init fail !\n",__FUNCTION__);
	}
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_MX6SL_NTX;
...
#ifndef BOARD_EARLY_INIT //[
	board_early_init();
#endif //] BOARD_EARLY_INIT
