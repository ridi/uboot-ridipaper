...
#ifdef CONFIG_MX6DL
	return (gd->bd->bi_dram[0].size+gd->bd->bi_dram[1].size);
#else
...
#ifdef CONFIG_MX6DL_NTX //[
		{
			DECLARE_GLOBAL_DATA_PTR;
			printf("reserved ram size=0x%lX\n",gdwRAM_ReservedSize);
			if(gd->bd->bi_dram[1].size) { 
				gd->bd->bi_dram[1].size -= (((gdwRAM_ReservedSize>>20)+1)<<20);
			}
			else {
				gd->bd->bi_dram[0].size -= (((gdwRAM_ReservedSize>>20)+1)<<20);
			}
		}
#else //][!CONFIG_MX6DL_NTX
...
static unsigned char * NtxHiddenMem_load_ntxbin(NtxHiddenMem *IO_ptNtxHiddenMem,unsigned long *O_pdwBinSize)
{
	...
	if(0==gpbRAM_TopAddr) {
		...
		{
			DECLARE_GLOBAL_DATA_PTR;
			gpbRAM_TopAddr = (unsigned char*)gd->bd->bi_dram[1].start + gd->bd->bi_dram[1].size;
		}
#else
...
#if defined(CONFIG_MX6SL_NTX) || defined(CONFIG_MX6DL_NTX) //[
...
static const NTX_GPIO gt_ntx_gpio_bat_low= {
	MX6DL_PAD_KEY_COL4__GPIO_4_14_INT,  // pin pad/mux control .
	4, // gpio group .
	14, // gpio number .
	0, // NC .
	0, // not inited .
		...
		char *O_pszKernCmdLine,unsigned long I_dwKernCmdLineSize)
{
...
#ifdef CONFIG_MFG
	if(gptNtxHwCfg) {
		strcat(O_pszKernCmdLine," boot_port=2");// SD3
	}
	...
	//
	// enable epdc ...
	sprintf(cCmdA," epdc");strcat(O_pszKernCmdLine,cCmdA);
